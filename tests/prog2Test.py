...
def test_program1(test):
    """het programma print na invoer van `hallo` dit woord nog eens 3x op één regel"""
    assert run('hallo') == "hallo hallo hallo\n"

@passed(checkstyle, forbidden_constructs)
def test_program2(test):
    """het programma print na invoer van `Toedeloe` dit woord nog eens 3x op één regel"""
    assert run('Toedeloe') == "Toedeloe Toedeloe Toedeloe\n"
